...
市面上的 humanizer 都在改用詞與句法。[StoryScope](https://arxiv.org/abs/2604.03136)（Russell et al., 2026：61,608 篇故事，人類＋5 個頂尖 LLM）證明：只用**敘事結構特徵**的分類器就能以 93.2% macro-F1 偵測 AI 小說，而把字句風格洗掉幾乎不影響偵測率（95.5% → 93.9%）。留下來的破綻全是架構性的：敘事者把主題講明、因果整齊的單線情節、情緒只用身體感官呈現、不引用真實世界、無視讀者、時間全程線性、結局靠主角的成長與釋懷收束。
...
貫穿全部的原則：**校準的目標是整個人類分布**。人類的實測值多落在中間值；反著做 AI 特徵是一種指紋，把每條規則用好用滿也是。skill 每篇挑 3–5 個手法，其餘留白。
